...
st.set_page_config(layout='wide', initial_sidebar_state='expanded',
                   page_title="SA CPI Forecast Dashboard",
                   page_icon=":chart_with_upwards_trend:")

"""
# :material/trending_up: South Africa CPI Nowcasting
...
def main():
    ...
    with st.sidebar:
        ...
        horizon = st.pills('Period for Percentage Change Computation (Months):',
    ...
    with tab_home:
        st.write('The predictions of this web application are powered by a Catboost\
                Regressor model trained on a time-series \
                of historic CPI values sourced from [Stats SA website](https://www.statssa.gov.za/) plus carefully engineered predictive features.\
                The model is retrained every month as new CPI data becomes available.')
        
        col1, col2, col3, col4 = st.columns((2,1,2,1),gap='small')
        ...
        col1.metric("Last Train Date", train_date,border=True)
        col2.metric("Model RMSE", model_metrics['rmse'],border=True)
        col3.metric("CPI Last Published for", hist_df['Date'].max().strftime('%B %Y'),
                    border=True)
        col4.metric("Headline CPI Change", f"{round(headline_yoy, 1)}%", delta="YoY",
                    #border=True
                    )
    ...
    with tab_visuals:
        ...
        fig = px.line(filtered_df, x='Date', y='Value', color='Category',
                       ...
                       line_dash='Data Type',markers=True)
        st.plotly_chart(fig, width='stretch')

        horizon = int(horizon.split()[0])
