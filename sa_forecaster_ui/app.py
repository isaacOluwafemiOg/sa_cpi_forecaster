...
def get_pct_change(ref_date,forecast_df,historical_df,months=1):
    ref_in_historical = ref_date in historical_df['Date'].values
    previous_date = ref_date - pd.DateOffset(months=months)
    prev_in_historical = previous_date in historical_df['Date'].values


    if ref_in_historical and prev_in_historical:
        ...
        ref_df = forecast_df.loc[forecast_df['Date'] == ref_date].reset_index(drop=True)
        ordered_ref_value = ref_df.sort_values('Category')['Value']
        prev_df = historical_df.loc[historical_df['Date'] == previous_date].reset_index(drop=True)
        ordered_prev_value = prev_df.sort_values('Category')['Value']

    elif not prev_in_historical:
        ref_df = forecast_df.loc[forecast_df['Date'] == ref_date].reset_index(drop=True)
        ordered_ref_value = ref_df.sort_values('Category')['Value']
        prev_df = forecast_df.loc[forecast_df['Date'] == previous_date].reset_index(drop=True)
        ordered_prev_value = prev_df.sort_values('Category')['Value']
...
def main():
    ...
    with tab_home:
        ...
        disp_current_forecast = pd.DataFrame(current_forecast)
        disp_current_forecast = disp_current_forecast.rename(columns={'Date': 'Forecast Date',
                                                             'Value': 'Predicted CPI Value'})
        disp_current_forecast = disp_current_forecast.sort_values(by='Forecast Date', ascending=False).reset_index(drop=True)
    ...
    with tab_visuals:
        ...
        num_months = st.slider('Number of Months:',
                                min_value=6,
                                max_value=hist_df['Date'].dt.month.nunique(),
                                step=1,value=12)
        
        # filter the dataframe based on selected categories and number of months
        filtered_df = hist_df[hist_df['Category'].isin(cat_options)]
        ...
        horizon = st.selectbox('Horizon for % change:', options=[1,3,6,12],
                                index=0,
                                help="Number of months to look back for percentage change calculation")
        pct_change_df = get_pct_change(ref_date_ch, current_forecast, hist_df, months=horizon)
    ...
    with tab_model_health:
        ...
        if features_importance:
            ...
            fig3 = px.bar(fi_df, x='Feature', y='Importance', title='Feature Importance')
